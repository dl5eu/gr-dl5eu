...
dvbt_pilot_gen::dvbt_pilot_gen(const dvbt_configure& c)
    : d_config(c),
      d_Kmin(d_config.d_Kmin),
      d_Kmax(d_config.d_Kmax),
      d_fft_length(d_config.d_fft_length),
      d_payload_length(d_config.d_payload_length),
      d_zeros_on_left(d_config.d_zeros_on_left),
      d_zeros_on_right(d_config.d_zeros_on_right),
      ...
      d_wk(d_Kmax - d_Kmin + 1)
{
    ...
    // Determine parameters from d_config file

    // Set-up pilot data depending on transmission mode
    if (d_config.d_transmission_mode == dtv::T2k) {
    ...
    } else if (d_config.d_transmission_mode == dtv::T8k) {
...
void dvbt_pilot_gen::format_tps_data()
{
    ...
    // Clause 4.6.2.5
    set_tps_bits(26, 25, d_config.d_constellation);
    // Clause 4.6.2.6
    set_tps_bits(29, 27, d_config.d_hierarchy);
    // Clause 4.6.2.7
    switch (d_config.d_code_rate_HP) {
    case dtv::C1_2:
    ...
    switch (d_config.d_code_rate_LP) {
    case dtv::C1_2:
    ...
    // Clause 4.6.2.8
    set_tps_bits(37, 36, d_config.d_guard_interval);
    // Clause 4.6.2.9
    set_tps_bits(39, 38, d_config.d_transmission_mode);
    // Clause 4.6.2.10
    if (d_frame_index % 2) {
        set_tps_bits(47, 40, d_config.d_cell_id & 0xff);
    } else {
        set_tps_bits(47, 40, (d_config.d_cell_id >> 8) & 0xff);
    }
